...
> **Поправка от 25.09.2026.** Приведённое здесь решение содержало дефект: `dayfirst=True` переставлял день и месяц и в дате формата ISO, так что строка «2001-02-03» читалась как 2 марта. Тест из следующего раздела этого не обнаружил, поскольку проверял готовый объект даты, а не текст. Разбор дефекта и исправление — в [следующем обновлении]({% link _articles/diploma_supplement_service/2026-09-25-revizija-defektov.md %}).
